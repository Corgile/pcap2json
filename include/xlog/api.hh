...
inline void
InstantiateFileLogger(Level level, std::string_view filename = logger_file,
                      size_t fileMaxSize = 100_KB, size_t maxFileCount = 1,
                      bool async = true, bool console = true,
                      bool alwaysFlush = false) {
  Logger<NameId>::Instance()->init(level, filename, fileMaxSize, maxFileCount,
                                   async, console, alwaysFlush);
}
